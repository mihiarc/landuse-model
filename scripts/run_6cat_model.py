#!/usr/bin/env python
"""
End-to-end script for 6-Category Land Use Transition Model

This script orchestrates the full pipeline:
1. Extract NRI transitions (crop/pasture/range/forest/urban with irrigation split)
2. Merge agricultural and urban rent data
3. Run multinomial logit estimation with mixed specification:
   - Ag/Urban: LCC + net returns
   - Range/Forest: LCC only

Usage:
    uv run python scripts/run_6cat_model.py

Or with options:
    uv run python scripts/run_6cat_model.py --sample 100000 --output results/test
    uv run python scripts/run_6cat_model.py --region SE --output output/southeast_6cat
"""
...
def run_pipeline(
    ...
    """
    Run the complete 6-category model pipeline.
    ...
    console.print(Panel.fit(
        "[bold blue]6-Category Land Use Transition Model[/]\n"
        "CR_IRR | CR_DRY | Pasture | Range | Forest | Urban\n\n"
        "[dim]Ag/Urban: LCC + Net Returns | Range/Forest: LCC only[/]",
        title="Pipeline"
    ))

    # Step 1: Extract NRI transitions (6 categories)
    console.print("\n[bold]Step 1: Extract NRI Transitions (6 categories)[/]")
    console.print("-" * 50)
    ...
    if skip_extraction and all_exist:
        ...
        from landuse.nri_extractor import extract_transitions_6cat
        extract_transitions_6cat(
            nri_file=nri_file,
    ...
    console.print("\n[bold]Step 3: Estimate Multinomial Logit Models (5 starting uses)[/]")
    console.print("-" * 50)

    from landuse.logit_estimation import main_6cat
    from landuse.region_specs import get_all_region_specs
    ...
    # Parse subregions filter (accepts comma-separated regions like "SE,DL")
    if region_filter:
        subregions = [r.strip() for r in region_filter.split(',')]
    else:
        subregions = None

    # Get region-specific model specifications
    region_specs = get_all_region_specs()
    ...
    main_6cat(
        ...
        start_range_file=str(transition_files['rg']),
        start_forest_file=str(transition_files['fr']),
...
def main():
    ...
    default_output = project_root / 'output' / '6cat_model'

    parser = argparse.ArgumentParser(
        description="Run 6-category land use transition model",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Run with sample of 100k NRI rows (for testing)
  uv run python scripts/run_6cat_model.py --sample 100000
  ...
  uv run python scripts/run_6cat_model.py --skip-extraction

Categories:
  CR_IRR  - Irrigated cropland (LCC + net returns)
  CR_DRY  - Non-irrigated cropland (LCC + net returns)
  ...
  UR      - Urban (LCC + net returns)
    ...
    parser.add_argument(
        '--lcc-only',
        action='store_true',
        help='Use LCC-only model for all uses (no net returns)'
    )
    parser.add_argument(
        '--region',
        type=str,
